...
use nomit\Database\Exception\InvalidArgumentException;
...
class SqlPreprocessor
{
	...
	public function process(array $params, bool $useParams = false): array
	{
		...
		while ($this->counter < count($params)) {
			...
			if (($this->counter === 2 && count($params) === 2) || !is_scalar($param)) {
				...
				throw new InvalidArgumentException('There are more parameters than placeholders.');
			}
		}
	...
	private function formatValue($value, ?string $mode = null): string
	{
		if (!$mode || $mode === self::ModeAuto) {
			...
			if (!is_string($value)) {
				$type = gettype($value);
				throw new InvalidArgumentException("Placeholder ?$mode expects string, $type given.");
			}
		...
		if ($mode && is_array($value)) {
			...
			$type = gettype($value);
			throw new InvalidArgumentException("Placeholder ?$mode expects array or Traversable object, $type given.");

		} elseif ($mode && $mode !== self::ModeAuto) {
			throw new InvalidArgumentException("Unknown placeholder ?$mode.");

		} else {
			throw new InvalidArgumentException('Unexpected type of parameter: ' . (is_object($value) ? $value::class : gettype($value)));
		}
	}
