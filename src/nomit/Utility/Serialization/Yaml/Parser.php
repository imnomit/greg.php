...
class Parser
{
    ...
    private function doParse(string $value, int $flags)
    {
            ...
            if ('-' === $this->currentLine[0] && self::preg_match('#^\-((?P<leadspaces>\s+)(?P<value>.+))?$#u', rtrim($this->currentLine), $values)) {
                ...
                if ('<<' === $key && (!isset($values['value']) || '&' !== $values['value'][0] || !self::preg_match('#^&(?P<ref>[^ ]+)#u', $values['value'], $refMatches))) {
                    ...
                    if (isset($values['value'][0]) && '*' === $values['value'][0]) {
                        ...
                        if (YamlParser::PARSE_OBJECT_FOR_MAP & $flags && $refValue instanceof \stdClass) {
                        ...
                        if (YamlParser::PARSE_OBJECT_FOR_MAP & $flags && $parsed instanceof \stdClass) {
                            $parsed = (array) $parsed;
                        }
                        ...
                        if (isset($parsed[0])) {
                            ...
                            foreach ($parsed as $parsedItem) {
                                if (YamlParser::PARSE_OBJECT_FOR_MAP & $flags && $parsedItem instanceof \stdClass) {
                ...
                if ($mergeNode) {
                    ...
                    if (!$this->isNextLineIndented() && !$this->isNextLineUnIndentedCollection()) {
                        ...
                        if ('<<' === $key) {
                            ...
                            if (YamlParser::PARSE_OBJECT_FOR_MAP & $flags && $value instanceof \stdClass) {
                                $value = (array) $value;
                            }
        ...
        if (YamlParser::PARSE_OBJECT_FOR_MAP & $flags && 'mapping' === $context && !\is_object($data)) {
            $object = new \stdClass();
    ...
    private function getLineTag(string $value, int $flags, bool $nextLineCheck = true): ?string
    {
        ...
        if (YamlParser::PARSE_CUSTOM_TAGS & $flags) {
            return $tag;
        }
