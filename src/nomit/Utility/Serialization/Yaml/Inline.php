...
namespace nomit\Utility\Serialization\Yaml;

use nomit\Utility\Serialization\Yaml\Exception\DumpException;
use nomit\Utility\Serialization\Yaml\Exception\ParseException;
use nomit\Utility\Serialization\Yaml\Tag\TaggedValue;
...
class Inline
{
    ...
    {
        self::$exceptionOnInvalidType = (bool) (YamlParser::PARSE_EXCEPTION_ON_INVALID_TYPE & $flags);
        self::$objectSupport = (bool) (YamlParser::PARSE_OBJECT & $flags);
        self::$objectForMap = (bool) (YamlParser::PARSE_OBJECT_FOR_MAP & $flags);
        self::$constantSupport = (bool) (YamlParser::PARSE_CONSTANT & $flags);
    ...
    public static function dump($value, int $flags = 0): string
    {
        switch (true) {
            case \is_resource($value):
                if (YamlParser::DUMP_EXCEPTION_ON_INVALID_TYPE & $flags) {
            ...
            case \is_object($value):
                ...
                if (YamlParser::DUMP_OBJECT & $flags) {
                    return '!php/object '.self::dump(serialize($value));
                }

                if (YamlParser::DUMP_OBJECT_AS_MAP & $flags && ($value instanceof \stdClass || $value instanceof \ArrayObject)) {
                    $output = [];
    ...
    {
        // array
        if (($value || YamlParser::DUMP_EMPTY_ARRAY_AS_SEQUENCE & $flags) && !self::isHash($value)) {
            $output = [];
    ...
    {
        if (YamlParser::DUMP_NULL_AS_TILDE & $flags) {
            return '~';
        }
            ...
            case \in_array($scalar[0], ['+', '-', '.'], true) || is_numeric($scalar[0]):
                ...
                switch (true) {
                    ...
                    case Parser::preg_match(self::getTimestampRegex(), $scalar):
                        ...
                        if (YamlParser::PARSE_DATETIME & $flags) {
                            return $time;
                        }
    ...
    private static function parseTag(string $value, int &$i, int $flags): ?string
    {
        ...
        if ('' === $tag || YamlParser::PARSE_CUSTOM_TAGS & $flags) {
            return $tag;
        }
