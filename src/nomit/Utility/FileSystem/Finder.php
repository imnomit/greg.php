...
class Finder implements \IteratorAggregate
{
    ...
    public function size(string $operator, ?int $size = null): static
    {
        ...
        return $this->filter(fn(FileInfo $file): bool => !$file->isFile() || Utility::compare($file->getSize(), $operator, $size));
    }
    ...
    public function date(string $operator, string|int|\DateTimeInterface|null $date = null): static
    {
        ...
        return $this->filter(fn(FileInfo $file): bool => !$file->isFile() || Utility::compare($file->getMTime(), $operator, $date));
    }
    ...
    private function traverseDir(string $dir, array $searches, array $subDirs = []): \Generator
    {
        if ($this->maxDepth >= 0 && count($subDirs) > $this->maxDepth) {
            ...
            throw new InvalidStateException("Directory '$dir' not found.");
        }
        ...
        foreach ($this->batches as $batch) {
            foreach ($batch->find as [$mask, $mode]) {
                if (FileSystem::isAbsolute($mask)) {
                    if ($batch->in) {
                        throw new InvalidStateException("You cannot combine the absolute path in the mask '$mask' and the directory to search '{$batch->in[0]}'.");
                    }
