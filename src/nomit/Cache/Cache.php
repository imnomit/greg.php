...
class Cache
{
	...
	public function __construct(StorageInterface $storage, ?string $namespace = null)
	{
	 ...
	 */
	final public function getStorage(): StorageInterface
	{
		return $this->storage;
	}
	...
	public function bulkLoad(array $keys, ?callable $generator = null): array
	{
		...
		$result = [];
		if (!$this->storage instanceof BulkReaderInterface) {
			foreach ($keys as $key) {
	...
	private function completeDependencies(?array $dp): array
	{
		...
		if (isset($dp[self::Expire])) {
			$dp[self::Expire] = DateTime::from($dp[self::Expire])->format('U') - time();
		}
