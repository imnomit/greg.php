...
use nomit\DependencyInjection\Definitions\Reference;
	...
	public static function expand(mixed $var, array $params, bool|array $recursive = false): mixed
	{
		...
		foreach ($parts as $n => $part) {
			if ($n % 2 === 0) {
				...
				if(is_array($val)) {
				    $val = Arrays::get($val, $part);
                } else if($val instanceof \nomit\Schema\DynamicParameter) {
                    $val = new DynamicParameter($val . '[' . var_export($part, true) . ']');
                } else {
                    throw new InvalidArgumentException(sprintf("Missing parameter '%s'.", $part));
                }
...
//				}
				...
				if ($val instanceof DynamicParameter) {
					...
					throw new InvalidArgumentException(sprintf("Unable to concatenate non-scalar parameter '%s' into '%s'.", $part, $var));
				}
