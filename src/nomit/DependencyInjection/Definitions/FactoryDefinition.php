...
namespace nomit\DependencyInjection\Definitions;

use nomit\DependencyInjection\Utilities;
use nomit\Exception\InvalidArgumentException;
use nomit\Generator as Php;
use nomit\Utility\Utility as NomitUtilities;
use nomit\Utility\Type;
...
final class FactoryDefinition extends Definition
{
	...
	public function setImplement(string $interface): static
	{
		...
		Utilities::ensureClassType(Type::fromReflection($method), "return type of $interface::create()", $this->getDescriptor());
	...
	public function resolveType(\nomit\DependencyInjection\Resolver $resolver): void
	{
		if (!$this->getType()) {
			throw new \nomit\DependencyInjection\Exception\ServiceCreationException('Type is missing in definition of service.');
		}
		...
		} catch (\nomit\DependencyInjection\Exception\ServiceCreationException $e) {
		...
		if ($type && !$type->allows($resultDef->getType())) {
			throw new \nomit\DependencyInjection\Exception\ServiceCreationException(sprintf(
		...
		if ($resultDef instanceof ServiceDefinition) {
			...
			if ($resultDef->getEntity() instanceof Reference && !$resultDef->getCreator()->arguments) {
				$resultDef->setCreator([ // render as $container->createMethod()
					new Reference(\nomit\DependencyInjection\ContainerBuilder::THIS_CONTAINER),
					\nomit\DependencyInjection\Container::getMethodName($resultDef->getEntity()->getValue()),
				]);
			}
		}
	...
	private function completeParameters(\nomit\DependencyInjection\Resolver $resolver): void
	{
		...
		foreach ($method->getParameters() as $param) {
			if (isset($ctorParams[$param->name])) {
				...
				$hint = NomitUtilities::getSuggestion(array_keys($ctorParams), $param->name);
				throw new \nomit\DependencyInjection\Exception\ServiceCreationException(sprintf(
	...
	public function generateMethod(Php\Method $method, \nomit\DependencyInjection\PhpGenerator $generator): void
	{
