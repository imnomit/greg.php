...
use nomit\DependencyInjection\Exception\NotAllowedDuringResolvingException;
use nomit\DependencyInjection\Exception\ServiceCreationException;
use nomit\Dumper\Dumper;
use nomit\Generator\Utilities as PhpHelpers;
use nomit\Utility\Arrays;
use nomit\Utility\Callback\Callback;
	...
	public function resolveEntityType(Statement $statement): ?string
	{
		...
		if (is_array($entity)) {
			...
			return $type && !in_array((string) $type, ['object', 'mixed'], true)
				? Utilities::ensureClassType($type, sprintf('return type of %s()', Callback::toString($entity)))
				: null;
		...
		try {
			...
			if (!str_contains($e->getMessage(), "\nRelated to")) {
				if (is_string($entity)) {
					...
					$desc = Utilities::entityToString($entity);
