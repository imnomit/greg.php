...
use nomit\Cryptography\Security\Utility;
 ...
 * @author   Anthony Ferrara <ircmaxell@ircmaxell.com>
 */
abstract class AbstractMixer implements \nomit\Cryptography\Entropy\Mixer
{
    ...
    public function mix(array $parts)
    {
        ...
        $len        = Utility::safeStrlen($parts[0]);
        ...
        return Utility::safeSubstr($result, 0, $len);
    }
    ...
    protected function normalizeParts(array $parts)
    {
        ...
        $callback  = function ($value) {
            return Utility::safeStrlen($value);
        };
    ...
    {
        $start = Utility::safeStrlen($string);
        $inc = Utility::safeStrlen($character);
        for ($i = $start; $i < $size; $i+= $inc) {
            $string = $string . $character;
        }

        return Utility::safeSubstr($string, 0, $size);
    }

    private function str_split($string, $size)
    {
        $blocks = array();
        $length = Utility::safeStrlen($string);
        $parts = ceil($length / $size);
        for ($i = 0; $i < $parts; $i++) {
            $blocks[] = Utility::safeSubstr($string, $i * $length, $length);
        }
