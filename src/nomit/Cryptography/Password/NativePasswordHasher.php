...
use nomit\Cryptography\Password\Exception\InvalidPasswordException;
...
final class NativePasswordHasher implements PasswordHasherInterface
{
    ...
    public function __construct(int $opsLimit = null, int $memLimit = null, int $cost = null, string $algorithm = null)
    {
        ...
        if (3 > $opsLimit) {
            throw new InvalidArgumentException('The "$opsLimit" argument must be 3 or greater.');
        }

        if (10 * 1024 > $memLimit) {
            throw new InvalidArgumentException('The "$memLimit" argument must be 10k or greater.');
        }

        if ($cost < 4 || 31 < $cost) {
            throw new InvalidArgumentException('The "$cost" argument must be in the range of 4-31.');
        }
