...
namespace nomit\Routing\Command;

/**
 * Import classes
 */

use nomit\Routing\Utilities\Utilities;
...
use function nomit\Routing\path_plain;
...
class RouteListCommand extends Command
{
    ...
    {
        $table = new \nomit\Console\Provider\Table\Table($output);
        ...
        foreach ($this->getRouter()->getRoutes() as $route) {
            $table->addRow([
                $route->getName(),
                $route->getHost() ?? 'ANY',
                Utilities::plainPath($route->getPath()),
