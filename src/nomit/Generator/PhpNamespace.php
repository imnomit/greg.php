...
final class PhpNamespace
{
	...
	{
		if ($name !== '' && !Utilities::isNamespaceIdentifier($name)) {
	...
	{
		if (
			!Utilities::isNamespaceIdentifier($name, true)
			|| (Utilities::isIdentifier($name) && isset(Utilities::Keywords[strtolower($name)]))
		) {
			throw new InvalidArgumentException("Value '$name' is not valid class/function/constant name.");

		} elseif ($alias && (!Utilities::isIdentifier($alias) || isset(Utilities::Keywords[strtolower($alias)]))) {
			throw new InvalidArgumentException("Value '$alias' is not valid alias.");
		}
		...
		if ($alias === null) {
			$base = Utilities::extractShortName($name);
			$counter = null;
			do {
				$alias = $base . $counter;
			...
			if (isset($aliases[$lower]) && strcasecmp($aliases[$lower], $name) !== 0) {
				...
				throw new InvalidStateException("Name '$alias' used already for '$this->name\\{$used[$lower]->getName()}'.");
			}
		}
	...
	{
		if (isset(Utilities::Keywords[strtolower($name)]) || $name === '') {
			return $name;
		} elseif ($name[0] === '\\') {
		...
		if ($of !== self::NameNormal) {
			return $aliases[strtolower($name)]
				?? $this->resolveName(Utilities::extractNamespace($name) . '\\') . Utilities::extractShortName($name);
		}
	...
	public function simplifyName(string $name, string $of = self::NameNormal): string
	{
		...
		if ($of !== self::NameNormal) {
			...
			return $this->simplifyName(Utilities::extractNamespace($name) . '\\') . Utilities::extractShortName($name);
		}
	...
	public function add(ClassLike $class): static
	{
		$name = $class->getName();
		if ($name === null) {
			throw new InvalidArgumentException('Class does not have a name.');
		}

		$lower = strtolower($name);
		if (isset($this->classes[$lower]) && $this->classes[$lower] !== $class) {
			throw new InvalidStateException("Cannot add '$name', because it already exists.");
		} elseif ($orig = array_change_key_case($this->aliases[self::NameNormal])[$lower] ?? null) {
			throw new InvalidStateException("Name '$name' used already as alias for $orig.");
		}
